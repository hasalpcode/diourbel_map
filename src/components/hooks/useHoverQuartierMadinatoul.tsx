...
import { quartierLayer } from "../sidebar/checkboxes/QuartCheckbox";
import '../application.css';
import { quartierMadinatoulLayer } from "../sidebar/checkboxes/QuartMadinatoulCheckbox";
...
export function useHoverQuartierMadinatoul(checked: boolean) {
  ...
  function handlePointerMove(e: MapBrowserEvent<PointerEvent>) {
    ...
    map.forEachFeatureAtPixel(
      ...
      { hitTolerance: 10, layerFilter: (l) => l === quartierMadinatoulLayer  },
    );

    if (features.length === 1 && checked) {
      setActiveFeature(features[0] as AirportFeature);
      overlayElement.innerHTML = `<div class="myy">${features[0].getProperties().name}</div>`;
